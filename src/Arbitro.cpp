...
void Arbitro::iniciarJuego() {
	...
	while (!validar) {
		...
		if (fgets(caracter, sizeof caracter, stdin) != NULL) {
			...
			if (strlen(caracter) == 0)
				noNumero = true;
			for (p = 0; p < strlen(caracter); p++)
				if (!isdigit(caracter[p]))
					noNumero = true;
			...
			if (!noNumero) {
				...
				cout << "INGRESE UN NUMERO MAYOR O IGUAL A 2 Y MENOR QUE 15:"
						<< endl << endl;
		}
	}
	...
	while (puntaje > 0) {
		...
		while (igual) {
			...
			while (!validar) {
				...
				if (fgets(caracter, sizeof caracter, stdin) != NULL) {
					...
					if (ingresoX >= dimension) {
						cout << "LA FILA TIENE QUE SER MENOR A"
								<< (tablero.getDimension()) << endl;
						noNumero = true;
					}
					...
					if (!noNumero) {
						...
						cout << "INGRESE NUMEROS MENORES A LA DIMENSION."
								<< endl << endl;
				}
			}
			...
			validar = false;
			while (!validar) {
				cout << "INGRESE LA COLUMNA DONDE CREA QUE ESTA LA BOLITA:"
						<< endl;
				if (fgets(caracter, sizeof caracter, stdin) != NULL) {
					caracter[strlen(caracter) - 1] = '\0';
					noNumero = false;
					...
					if (ingresoY >= dimension) {
						cout
								<< "TIENE QUE INGRESAR UN NUMERO MENOR A LA DIMENSION."
								<< endl;
						noNumero = true;
					}
					if (!noNumero) {
...
bool Arbitro::validarIntento(int fila, int columna) {
	...
	if (fila == filaBolita && columna == columnaBolita) {
		...
		tablero.setCasilla(fila, columna, 'O');
		puntaje = floor(
				puntaje
						- (100
								/ (tablero.getDimension()
										* tablero.getDimension())));
		cout << "NO ES LA POSICION DE LA BOLITA, SU PUNTAJE ES: " << puntaje
				<< endl;
		cout << "DE LA DIMENSION INGRESADA, USTED ESTA A "
				<< calcularDistancia(fila, columna) << " CASILLA DE LA BOLITA. "
				<< endl;
		imprimirTablero();
	}
